...
const ScheduleListItem = ({
  ...
  return (
    <div onDoubleClick={() => onItemDubleClick(node.nodeId)}>
      <div className="flex justify-between items-center mb-2 select-none cursor-pointer">
        <strong className="w-[65%] truncate">{node.nodeTitle}</strong>
        <span className="w-[35%] text-xs text-right font-semibold">
          {formattedDate}
        </span>
      </div>
